...
const processTaobaoLink: URLOptimizerProcessorHandler = async (url, options, env) => {
...
const processTaobaoShortLink: URLOptimizerProcessorHandler = async (url, options, env) => {
    ...
    return await processTaobaoLink(match[1], options, env);
}
